CREATE OR REFRESH STREAMING TABLE fact_reviews (
...
WITH model_response AS (
                        SELECT 
                          *,
                          ...
                          FROM STREAM landing_review_incremental
  )
  SELECT 
    review_id,
    order_id,
    ...
    review_timestamp,
    TO_DATE(review_timestamp) AS review_date,
    DATE_FORMAT(review_date, 'yyyy-MM') AS review_year_month,
    DATE_FORMAT(review_date, 'MMM') AS review_month,
    DATE_FORMAT(review_date, 'yyyy') AS review_year,
    DATE_FORMAT(review_date, 'EEEE') AS review_day_of_week,
    (CASE WHEN review_day_of_week IN ('Sunday', 'Saturday') THEN TRUE ELSE FALSE END) AS is_weekend
  FROM model_response;